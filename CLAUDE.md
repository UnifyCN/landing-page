...
- **Programmatic guide clusters** (`/teer`, `/health-card`, `/drivers-licence`) — the non-brand SEO engine. Each is one typed data file in `src/lib/` (`teer.ts`, `health-coverage.ts`, `licence-exchange.ts`), one hub `index.astro`, and one `[slug].astro` detail route, all prerendered, all emitting `BreadcrumbList` + `FAQPage` JSON-LD and linking to `/` with the anchor "newcomer settlement app". The two province clusters share `src/styles/cluster.css` (`.cl-*` classes). Facts carry a `lastVerified` month and an `officialUrl`; when a provincial rule changes, edit the entry and the date, never the template. `/teer/[tier]` builds its `<title>`/H1 from the live NOC count in `src/lib/noc-2021.ts`, which is GENERATED by `scripts/build-noc-data.mjs` from the Statistics Canada NOC 2021 CSV (do not hand-edit). New hubs go in the Footer `navigate[]` and in `tests/smoke.spec.ts`. The weekly blog automation prefers topics that deepen one of these hubs (see `docs/weekly-blog-automation.md`).
