...
- **Supabase (read-only)** — `/events` reads `public.events` from the shared Unify app DB (`wrbauxutkysljmsqojts`, same DB as the mobile + web apps) over PostgREST with the publishable key (`src/lib/events/client.ts`; public-read RLS). All reads live in `src/lib/events/queries.ts`. This site never writes.
...
│   ├── about.astro
│   ├── _community.astro         # ARCHIVED (unrouted); /community 301s to /events
│   ├── events/                  # SSR: index (agenda) + [event] (<id>-<slug> detail)
│   ├── contact.astro
│   ├── partners.astro
...
- **All static pages must `export const prerender = true`** so they bake to HTML at build time and are served from the Cloudflare CDN edge. The repo had every page hitting the worker on every request before this — TTFB on every navigation. Currently prerendered: `/`, `/about`, `/contact`, `/partners` (+ all `/partners/[slug]` via `getStaticPaths`), `/resources` (+ all `/resources/[slug]`). SSR retained for `/blog/*` (live Sanity fetch), `/events/*` (live Supabase fetch) and `/api/*`. There are no local `/privacy` or `/terms` routes — the Footer links those to external Notion pages, see the Footer section below.
...
- **Categories**: `post.category` is a required single-value string (10 options). The canonical list lives in `src/lib/blog-categories.js` (plain JS so the Studio schema, the frontend, and the Node scripts all import the same file - never add a second list). Values are stored in Sanity; renaming one needs a backfill (`scripts/backfill-post-categories.mjs` is the pattern). `/blog` renders one chip per category and pre-applies `?category=` server-side (canonical stays `/blog`); posts with a missing/unknown value fall back to `others`. Tie-break rule for editors and the automation: audience wins - a post for international students is `international-students` even if it is about banking or jobs.
...
**Rule:** when you add a new interactive island, or rename a binding-guard attribute (`data-nav-bound`, `data-faq-bound`, `data-cf-bound`, `data-bp-bound`, `data-ah-carousel-init`, `data-bf-bound` (blog filter), `data-rv-bound` (resources filter), `data-ev-bound` (events agenda)), add or update its spec in the same change.
...
- Nav links (order): Home | About ▾ (Partners, Contact) | Events | Resources ▾ (Blog). Max five top-level tabs; "Find Services" joins later. A dropdown parent still links to its own page; its caret button opens the menu (keyboard/touch), hover opens it for pointers, and the parent reads active on its children's pages (`data-match`). Mobile overlay lists children indented under their parent.
- Full inline nav from **1024px** (`--breakpoint-nav-full`, navbar only); the rest of the site keeps desktop = 1400px.
...
- **Navigate column:** Home | About | Events | Blog | the guide hubs | Contact. Events replaced Community (Sept 2026) when `/community` was archived.
...
- **Partners** — white bg, CSS-only marquee of 18 logos doubled for a seamless loop, mask-gradient fade edges, pauses on `:hover`, respects `prefers-reduced-motion`. Each logo is an `<a>` to `/partners/[slug]` — the array is `import { partners } from "../../lib/partners"` (same source as `PartnersGrid.astro`, so the slug, logo path, and name come from one place). The duplicated loop set carries **both** `aria-hidden="true"` and `tabindex="-1"` (an `aria-hidden` element that's still focusable is an a11y violation); first set is focusable with a `:focus-visible` brand-red ring.
- **Journey ("Key Benefits")** — white bg, eyebrow "Core Features" + centered H2. **Four** feature blocks: Checklist (cream `#f5eeda`) / AI Companion (blue-grey `#e9ecf6`, reversed) / Community (peach `#f5e6d2`) / Learn (yellow-green `#ecf0c2`, reversed). Each card holds a transcoded H.264 phone demo from `public/assets/demos/` (`checklist-tab-demo.mp4`, `ai-tab-demo.mp4`, `social-tab-demo.mp4`, `learn-tab-demo.mp4`) — sources are HEVC `.mov` originals, transcoded with macOS `avconvert --preset Preset1920x1080` (no ffmpeg needed). Each `<video>`'s `poster=` MUST point to the demo's actual frame 0 at `public/assets/demos/posters/<demo-name>.avif` — when poster pixels match the first decoded frame the play-start swap is invisible; an off-brand screenshot poster (the prior state) produced a visible flash. Regenerate posters whenever a demo is re-recorded: AVFoundation Swift one-liner (`AVAssetImageGenerator.copyCGImage(at: .zero …)`) → PNG → Sharp `avif({ quality: 55 })`. The Community card's `transform: scale(1.09)` applies to the poster too, so a frame-0 extract stays matched automatically. **Card `aspect-ratio` MUST match each clip's native ratio** so `object-fit: cover` never crops the phone — base is 3:4 (matches the first three 810×1080 clips); `--learn` overrides to `4 / 5` for its 864×1080 clip. At desktop **≥1280px** the grid switches from `1fr 1fr` to `auto minmax(0, 560px)` + `justify-content: center` (reversed swaps the columns) so the text+card pair sits as a centred unit and the middle gap collapses to just `column-gap`. Community gets two extra knobs: `.feature-text { max-width: 26rem }` to keep its short heading from floating away from the card, and `transform: scale(1.09); transform-origin: top center` on its `.feature-card-img` to crop the ~92px of empty whitespace below the phone in the `social-tab-demo` recording (until that clip is re-framed to fill the frame the way the other three do). IntersectionObserver reveal wired through `astro:page-load`.
...
- **CTABand** — dark (`var(--color-ink)`) band used on About/Events/Contact/Partners, **NOT the homepage**. "Stop guessing. / Start settling." headline, brand-red "Download the App Now" button, scroll fade-up via IntersectionObserver.
...
- **Programmatic guide clusters** (`/teer`, `/health-card`, `/drivers-licence`, `/credentials`) — the non-brand SEO engine. Each is one typed data file in `src/lib/` (`teer.ts`, `health-coverage.ts`, `licence-exchange.ts`, `credentials.ts`), one hub `index.astro`, and one `[slug].astro` detail route, all prerendered, all emitting `BreadcrumbList` + `FAQPage` JSON-LD and linking to `/` with the anchor "newcomer settlement app". Every cluster except `/teer` shares `src/styles/cluster.css` (`.cl-*` classes; `.cl-table--fit` for 2-column tables that must fit a phone, `.cl-table--hub` + `.cl-col-extra` to drop repeated columns under 600px). Facts carry a `lastVerified` month and an official source; when a rule changes, edit the entry and the date, never the template. `/teer/[tier]` builds its `<title>`/H1 from the live NOC count in `src/lib/noc-2021.ts`, which is GENERATED by `scripts/build-noc-data.mjs` from the Statistics Canada NOC 2021 CSV (do not hand-edit).
  - **`/credentials/[profession]`** — 15 regulated professions (nurses, doctor, pharmacist, dentist, physio, med lab tech, engineer, accountant, lawyer, teacher, ECE, social worker, electrician, plumber), each with steps, exams, and the regulator in all 13 provinces and territories (fixed order BC … NU). `required: false` marks rows where the job is not regulated or the trade is voluntary; `regulatedCount()` drives the "N of 13" pill, and `regulatedLabel` overrides it where the count misleads (accountant: only the CPA title is regulated). `credentialForNoc()` adds a "Trained abroad? How to get licensed" link to matching rows on every `/teer/[tier]` job list, so keep each profession's `nocCodes` exact. Entries carry a researcher `confidence`; re-verify "medium" ones first. Research was done in Sept 2026 against regulator sites; several regulators (EGBC, PEO, CPA Ontario, yukon.ca, gov.nu.ca) block automated reads.
  - **`/canadian-resume`** — a single pillar page (not a cluster) with its content in `src/lib/canadian-resume.ts`, sourced from Job Bank, IRCC, the Ontario Human Rights Commission, and Ontario's ESA job-posting rules. `/resources/how-to-write-a-resume` links to it via the `relatedGuide` field on `Resource`, so the video and the written guide do not compete.
  - **`/banking`** — a single pillar page (`src/pages/banking/index.astro`, content in `src/lib/newcomer-banking.ts`): documents, the federal right to a basic account (FCAC), and the newcomer packages of the major banks compared in one format. **RBC is a Unify partner**: keep the disclosure next to the comparison table, and keep every bank's row to the same fields (no extra praise for any bank). Bank offers change often; re-check `lastVerified` rows against each bank's page before relying on them.
  - New hubs go in the Footer `navigate[]`, `public/llms.txt`, `tests/smoke.spec.ts`, and `docs/newcomer-coverage-map.md`. The weekly blog automation now picks **gap topics first** from `docs/newcomer-coverage-map.md` and limits hub spokes to about 1 post in 4 (see `docs/weekly-blog-automation.md`).
...
- **Scripts:** `scripts/{gsc-fetch,list-post-slugs,fetch-pexels,notify-run,update-thumbnail}.mjs`. `create-post.mjs` has a slug-overwrite guard (refuses to `--publish` over an existing published slug unless `--force`) and refuses to commit without a valid `category`.
